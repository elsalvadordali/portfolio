...
export const WIDTH = browser ? window.innerWidth > 700 ? window.innerWidth + 35 : window.innerWidth : 1000;
export const HEIGHT = browser ? window.innerHeight * 2 : 1000;
...
export function generateLilyPads(i, j, runningTwice) {
    const lilyPad = {
        x: 0,
        y: 0,
...
function returnDistance(clickCoord, lilyPad) {
    return Math.sqrt((clickCoord[0] - lilyPad.x) ** 2 + (clickCoord[1] - lilyPad.y) ** 2);
}
export function changeColor(lilyPad) {
    lilyPad.colorIndex = Math.floor(Math.random() * 10) + 9;
    return lilyPad;
}
export function wave(e, lilyPads, waves) {
    ...
    for (let i = 0; i < lilyPads.length; i++) {
        ...
        if (lilyPads[i].distance < lilyPads[closestLilyPad].distance)
            closestLilyPad = i;
